...
#ifndef _XBOX
		// Fire mod events when player has actually joined the world (level set + player added).
		ModEvents::EmitPlayerJoin(0);
		if (level->getLevelData()) {
			ModEvents::EmitWorldLoadedW(level->getLevelData()->getLevelName());
			ModEvents::EmitJoinWorldW(0, level->getLevelData()->getLevelName());
		} else {
			ModEvents::EmitWorldLoaded("");
			ModEvents::EmitJoinWorldW(0, L"");
		}
#endif
	}
	else
