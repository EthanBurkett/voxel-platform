...
UIScene_SettingsMenu::UIScene_SettingsMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
    ...
    // Win64: use Button5 for "Player name" (Iggy movie only has 6 buttons; no Button7)
    m_buttons[BUTTON_ALL_UI].init(UIString(L"Player name"), BUTTON_ALL_UI);
#else
...
void UIScene_SettingsMenu::handlePress(F64 controlId, F64 childId)
{
    ...
    case BUTTON_ALL_UI:
#if defined(_WINDOWS64)
        ui.NavigateToScene(m_iPad, eUIScene_PlayerNameMenu);
#else
