...
#include "ModLoader/engine/mod_log.h"
#include "ModLoader/engine/node_host.h"
#include "ModLoader/modules/engine_module.cpp"  // static registration
#include "ModLoader/modules/mc_module.cpp"
                       ...
                       _In_ int nCmdShow)
{
    ...
    // Initialize Node (mod loader): generates types/, then run event loop on a background thread so mods load.
    if (!launchOptions.serverMode)
    {
        static char argv0[] = "MinecraftClient";
        char *argv[] = { argv0, nullptr };
        if (g_nodeHost.Init(1, argv))
        {
            std::thread modThread([]() {
                g_nodeHost.Run();
            });
            modThread.detach();
            ModLog("[ModLoader] Mod loader thread started — mods/ will be loaded.\n");
        }
        else
        {
            ModLog("[ModLoader] Node init failed — mods will not load.\n");
        }
    }
