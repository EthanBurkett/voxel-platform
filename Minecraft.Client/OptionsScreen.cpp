...
void OptionsScreen::init()
{
    ...
    buttons.push_back(new Button(PLAYER_NAME_BUTTON_ID, width / 2 - 100, height / 6 + 24 * 6 + 12, L"Player name"));
...
void OptionsScreen::buttonClicked(Button *button)
{
    ...
    if (button->id == PLAYER_NAME_BUTTON_ID)
    {
        wstring currentName = minecraft->user != nullptr ? minecraft->user->name : L"Player";
        minecraft->setScreen(new PlayerNameScreen(this, currentName));
    }
    if (button->id == 200)
	{
        minecraft->options->save();
